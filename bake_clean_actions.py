	...
	while new_verts:
		...
		for newIndex in range(len(new_vert_indices) - 1):
			...
			for i, point in enumerate(splineVerts[new_vert_indices[newIndex] + 1:new_vert_indices[newIndex + 1]]):
				alti = altitude(splineVerts[new_vert_indices[newIndex]], splineVerts[new_vert_indices[newIndex + 1]],
								point)
				if alti > alti_store:
					alti_store = alti
...
def clean_group(group, type_id):
	...
	# now clean every curve, i.e. remove all keys we don't need
	for fcurve in fcurves:
...
def bake_and_clean(error=0.25, exp_power=2):
	logging.info(f"Started baking and cleaning process with max error {error}")
	starttime = time.time()
	...
	if bpy.data.actions:
		...
		if src_armatures:
			...
			if armature.pose is None:
				return
			logging.info(f"Found armature {armature.name} with constraints")
			...
			logging.debug(f"Error margins for each bone: {errors}")

			arm_ob_name = armature.name[1:]
			arm_name = armature.data.name[1:]
			if arm_ob_name in bpy.data.objects:
				armature_copy = bpy.data.objects[arm_ob_name]
				armature_copy.data = armature.data.copy()
				armature_copy.data.name = arm_name
				logging.debug(f"Armature {armature_copy.name} without constraints exists")
			else:
			...
			for name in armature_copy.pose.bones.keys():
				armature_copy.driver_remove(f'pose.bones["{name}"].rotation_quaternion')
			...
			for action in src_actions:
				...
				for name, p_bone in armature_copy.pose.bones.items():
					...
					for (f, matrix) in zip(frame_range, pose_info):
						p_bone.matrix_basis = matrix[name].copy()
						p_bone.keyframe_insert("location", index=-1, frame=f, group=name)

						rotation_mode = p_bone.rotation_mode
						if rotation_mode == 'QUATERNION':
							p_bone.keyframe_insert("rotation_quaternion", index=-1, frame=f, group=name)
						elif rotation_mode == 'AXIS_ANGLE':
							p_bone.keyframe_insert("rotation_axis_angle", index=-1, frame=f, group=name)
						else:  # euler, XYZ, ZXY etc
							if euler_prev is not None:
								euler = p_bone.rotation_euler.copy()
								euler.make_compatible(euler_prev)
								p_bone.rotation_euler = euler
								euler_prev = euler
								del euler
							else:
								euler_prev = p_bone.rotation_euler.copy()
							p_bone.keyframe_insert("rotation_euler", index=-1, frame=f, group=name)

						p_bone.keyframe_insert("scale", index=-1, frame=f, group=name)
				logging.info(f"Cleaning {copy_action.name}")
			...
			bpy.context.view_layer.update()
			logging.info(f"Finished action baking and fcurve cleaning in {time.time() - starttime:.2f} seconds")
		else:
