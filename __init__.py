bl_info = {
	"name": "Blue Fang BFB format (Zoo Tycoon 2)",
	"author": "HENDRIX",
	"blender": (4, 4, 0),
...
class ToggleIKLink(bpy.types.Operator):
	...
	ik_names: StringProperty(name="Limb IK bone names", description="The names of the bones to (un)link.",
							 default="*IKH.L,*IKH.R,*IKF.L,*IKF.R")
...
def menu_func_export(self, context):
	icon = preview_collection["zt2.png"].icon_id
	self.layout.operator(ExportBFB.bl_idname, text="Blue Fang Model (.bfb)", icon_value=icon)
	self.layout.operator(ExportDAT.bl_idname, text="Blue Fang Map (.dat)", icon_value=icon)
	self.layout.operator(ExportBF.bl_idname, text="Blue Fang Animation (.bf)", icon_value=icon)


def menu_func_import(self, context):
	icon = preview_collection["zt2.png"].icon_id
	self.layout.operator(ImportBFB.bl_idname, text="Blue Fang Model (.bfb)", icon_value=icon)
...
def menu_func_armature(self, context):
	self.layout.operator(AddCorrectionBone.bl_idname, text="Add Correction Bone",
						 icon_value=preview_collection["zt2.png"].icon_id)
	self.layout.operator(ReorientBone.bl_idname, text="Reorient Bone", icon_value=preview_collection["zt2.png"].icon_id)
	self.layout.operator(ToggleIKLink.bl_idname, text="Toggle IK Link",
						 icon_value=preview_collection["zt2.png"].icon_id)
...
def register():
	...
	bpy.types.VIEW3D_MT_mesh_add.append(menu_func_add_objects)
	bpy.types.VIEW3D_PT_tools_armatureedit_options.append(menu_func_armature)
	bpy.types.VIEW3D_PT_tools_weightpaint_options.append(menu_func_weights)
	bpy.types.VIEW3D_PT_tools_object_options.append(menu_func_weights)
...
def unregister():
	...
	bpy.types.VIEW3D_MT_mesh_add.remove(menu_func_add_objects)
	bpy.types.VIEW3D_PT_tools_armatureedit_options.remove(menu_func_armature)
	bpy.types.VIEW3D_PT_tools_weightpaint_options.remove(menu_func_weights)
	bpy.types.VIEW3D_PT_tools_object_options.remove(menu_func_weights)
