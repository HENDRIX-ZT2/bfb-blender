	...
	for b in range(0, blockcount):
		...
		if typeid == 1:
			...
			for key in mesh_data:
				if key == "tri":
					mesh[key] = mesh_data[key][int(t_sta/3):int((t_sta+t_num)/3)]
				else:
			...
			if typeid == 8:
				...
				if not armature:
					...
					#iter_unpack seems to be broken - size calculated for the format string is wrong
					#for boneid, parentid, bonegroup, bonename, m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32, m33 in iter_unpack("3b 64s 16f", datastream[pos : pos+numbones*131]):
					for x in range(0, numbones):
						boneid, parentid, bonegroup, bonename = unpack_from("3b 64s", datastream, pos+x*131)
						m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32, m33 = unpack_from("16f", datastream, pos+67+x*131)
						#rotate the BFB bone-space matrix -> from_forward="X"
						bonematrix = mathutils.Matrix(((-m10, -m11, -m12, -m13),
													   ( m00,  m01,  m02,  m03),
													   ( m20,  m21,  m22,  m23),
													   ( m30,  m31,  m32,  m33)))
						#create a bone
						bone = armData.edit_bones.new(bfbname_to_blendername(bonename))
						#parent it and get the armature space matrix
						if parentid > 0:
							bone.parent = armData.edit_bones.values()[parentid-1]
							#multiply the rotated bone-space matrix with the backrotated (= original BFB orientation) parent matrix to get BFB armature-space matrix
							#to_x = swap and invert the first rows
							bonematrix *= to_x * bone.parent.matrix.transposed()
						#blender transposes matrices
						bonematrix.transpose()
						#set transformation
						bone.head = mathutils.Vector((0,0,0))       
						bone.tail = mathutils.Vector((0,1,0))
						bone.transform(bonematrix.to_3x3())
						bone.translate(bonematrix.to_translation())
					#fix the bone length
