...
def write_nodes(dirname, action, nodes, fps):
	stream = b''.join([struct.pack('=I f I', 2, action.frame_range[1] / fps, len(nodes)), b"".join(nodes),
					   struct.pack('=f H 6s f H 12s', 0.0, 6, b"start", action.frame_range[1] / fps, 5, b"end ")])
	...
	if armature:
		try:
			...
			for action in bpy.data.actions:
				...
				if anim_start != 0:
					errors.append(
						"Action " + action.name + " did not start at frame 0! This has been automatically fixed!")
				...
				if "*" + action.name in bpy.data.actions and "secondary_" in action.name.lower():
					errors.append(
						"Action " + action.name + " uses only bones keyframed in the original to avoid slithering.")
					...
					if group.name in bones_data:
						...
						for fcurves in (rotations, translations, eulers):
							...
							if not same_amount_of_keys:
								...
								rotations = [fcurve for fcurve in group.channels if
											 fcurve.data_path.endswith("quaternion")]
								translations = [fcurve for fcurve in group.channels if
						...
						if rotations:
							if len(rotations) != 4:
								...
								for i in range(0, num_keys):
									...
									quat = export_keymat(rest_rot, mathutils.Quaternion(
										[fcurve.keyframe_points[i].co[1] for fcurve in
										 rotations]).to_matrix().to_4x4()).to_quaternion()
									key_bytes.append(struct.pack('=H4h', rint(frame / fpms), rint(quat.x * 10000),
																 rint(quat.y * 10000), rint(quat.z * 10000),
																 rint(quat.w * 10000)))
						...
						if translations:
							if len(translations) != 3:
								...
								for i in range(0, num_keys):
									...
									trans = export_keymat(rest_rot, mathutils.Matrix.Translation(
										[fcurve.keyframe_points[i].co[1] for fcurve in
										 translations])).to_translation() + rest_trans
									key_bytes.append(struct.pack('=H3h', rint(frame / fpms), rint(trans.x * 1000),
																 rint(trans.y * 1000), rint(trans.z * 1000)))
						...
						if eulers:
							if len(eulers) != 3:
								...
								for i in range(0, num_keys):
									...
									quat = export_keymat(rest_rot, mathutils.Euler(
										[fcurve.keyframe_points[i].co[1] for fcurve in
										 eulers]).to_matrix().to_4x4()).to_quaternion()
									key_bytes.append(struct.pack('=H4h', rint(frame / fpms), rint(quat.x * 10000),
																 rint(quat.y * 10000), rint(quat.z * 10000),
																 rint(quat.w * 10000)))
						...
						nodes.append(struct.pack('=32s H 2B H I 2B', blendername_to_bfbname(group.name).encode('utf-8'),
				...
				for data_type in ("rotation_euler", "rotation_quaternion", "location", "scale"):
					...
					if fcurves:
						...
						for mod_id in mod_ids:
							...
							num_mod_types += 1
							key_bytes.append(
								struct.pack("=4H", mod_id, num_keys, 8 + num_keys * struct.calcsize(fmt), 0))
							for i in range(0, num_keys):
								...
								key_bytes.append(
									struct.pack(fmt, rint((fcurves[0].keyframe_points[i].co[0]) / fpms), *key))
				key_bytes = b"".join(key_bytes)
				nodes.append(
