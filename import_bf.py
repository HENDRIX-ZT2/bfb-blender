...
def read_bf_empties(dir, anim, info, fpms):
	...
	with open(os.path.join(dir, anim), 'rb') as f:
		...
		for bone in range(0, num_bones):
			...
			try:
				ob = bpy.data.objects[bone_name]
			except:
				ob = create_empty(None, bone_name, mathutils.Matrix())
			...
			if name in bones_data:
				...
				while pos < next_bone:
					...
					if data_type == "rotation_euler":
						...
						# initialize the fcurves
						fcurves = [action.fcurves.new(data_path='pose.bones["' + name + '"].' + data_type, index=i,
													  action_group=name) for i in range(0, len(key_i))]
					...
					for element in iter_unpack(fmt, datastream[8 + pos: 8 + pos + calcsize(fmt) * num_keys]):
						if data_type == "rotation_euler":
							...
							if data_type == "scale":
								...
								key = import_keymat(rest_rot_inv, mathutils.Matrix.Translation(mathutils.Vector(
									[element[x] / scale_fac for x in key_i]) - rest_trans)).to_translation()
							elif data_type == "rotation_quaternion":
								key = import_keymat(rest_rot_inv, mathutils.Quaternion(
									[element[x] / scale_fac for x in key_i]).to_matrix().to_4x4()).to_quaternion()
							# add the keys now
					...
					if mod_identifier == 8:
						# initialize the fcurves
						fcurves = [action.fcurves.new(data_path='pose.bones["' + name + '"].' + data_type, index=i,
													  action_group=name) for i in (0, 1, 2)]
						...
						for x, y, z, t in zip(x_r, y_r, z_r, ts):
							key = import_keymat(rest_rot_inv,
												mathutils.Euler((x, y, z)).to_matrix().to_4x4()).to_euler()
							for fcurve, k in zip(fcurves, key):
