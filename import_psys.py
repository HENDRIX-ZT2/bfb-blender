...
def create_material(reporter, filepath, ob, matname):
				...
				for i in range(0, 2):
					if name == "TextureTransform" + str(i):
						for key in param.find("./offsetu"):
							...
							mat.texture_slots[i].keyframe_insert("offset", index=0,
																 frame=int(float(key.attrib["time"]) * fps))
						for key in param.find("./offsetv"):
							mat.texture_slots[i].offset[1] = float(key.attrib["value"])
							mat.texture_slots[i].keyframe_insert("offset", index=1,
																 frame=int(float(key.attrib["time"]) * fps))
			#multi-textures
			for i in range(0, 2):
				if name == "Texture" + str(i):
					...
					try:
						...
						reporter.show_warning(
							f"{name} in Materials/{matname}.BFMAT has no image reference! Add the image name in the .BFMAT to fix!")
	else:
...
def getv(param):
	return [float(v) for v in param.text.split(",")]
...
def load(reporter, filepath=""):
	try:
		...
		print(f"{filepath} cannot be parsed, likely due to an XML syntax error!")
		return {'FINISHED'}
	...
	for block in simulator:
				...
				if param.tag == "param":
					...
					if param.attrib["label"] == "transform":
						matrix = mathutils.Matrix(
							(getv(param)[0:4], getv(param)[4:8], getv(param)[8:12], getv(param)[12:16]))
		...
		if block.tag == "renderer":
			...
			for param in block:
				...
				# 	material = create_material(reporter, filepath, emitter, param.text)
